import numpy as np
import PS3.fixed_income_derivatives_E2025 as fid
...
### Other options for interpolation_options: "Method" = "hermite, spline, and some others"
...
print(T_fit, T_fit.shape, type(T_fit))
print(R_fit, R_fit.shape, type(R_fit))
...
print(T_inter)
print(T_inter.shape,R_inter.shape)
...
### S is the accruel factor. This curve repruduces the market par swap rate that we have on slides. 
...
### so here we see that if we shift all the zcb rates by 1bp, then the swap rate also increases by approx 1bp. 
...
## Here we are only shifting the 5Y zcb rate by 1bp, and we see that the swap rate increases by approx 0.9bp. Or all most the same as before. 

## Now we are reduing the thing by where we just make one jump in the 5Y swap market rate. 
...
### Here we see that if we just increase the 5Y swap market rate by 1bp, then the swap rate also increases by approx 1bp. Which is what we would expect.
