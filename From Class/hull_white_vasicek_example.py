...
p1 = ax.scatter([i for i in range(1,N_simul_swaption+1)], price_swaption_plot, s = 1, color = 'black', marker = ".",label=f"Swaption price in bps: {np.round(price_swaption_plot[-1],2)}")
plots = [p1]
labels = [item.get_label() for item in plots]
ax.legend(plots,labels,loc="upper right",fontsize = 5)
...
plt.grid(axis = 'y', which='major', color=(0.7,0.7,0.7,0), linestyle='--')
ax.set_ylabel(f"Swaption price",fontsize = 7)
p1 = ax.scatter([i for i in range(1,N_simul_digital+1)], price_digital_plot, s = 1, color = 'black', marker = ".",label=f"Digital option price: {np.round(price_digital_plot[-1],4)}")
plots = [p1]
labels = [item.get_label() for item in plots]
ax.legend(plots,labels,loc="upper right",fontsize = 5)
